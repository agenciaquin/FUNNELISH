# CORRECCIONES V59 — Stats tiles se actualizan con filtro de fecha en Billetera QUINO
...
|---------|-----------|---------|
| `billetera.js` | `calcularStats()` aplica filtro de fechas al base | Los tiles mostraban totales globales aunque hubiera fechas filtradas |
| `billetera.js` | Event listeners de fechas llaman `actualizarUI()` en vez de `renderTabla()` | `renderTabla()` no actualiza los tiles — solo la tabla |
| `billetera.js` | `renderTabla()` usa `s.base` para filtro 'todos' | Elimina duplicado del filtro de fechas (ya aplicado en `calcularStats`) |
| `billetera.js` | `stat-total` muestra "N en el período · M total" cuando hay fechas activas | Feedback visual de que los tiles están filtrados |

---

## Instrucciones para Claude Code

> Aplica estos cambios al proyecto ConfirmaYa en `billetera.js`:
> 1. En `calcularStats()`, filtra `remisiones` por `fechaDesde`/`fechaHasta` antes de calcular entregadas/pendientes/devueltas. Agrega `base` al objeto retornado.
> 2. En `actualizarUI()`, muestra en `stat-total` el texto "N en el período · M total" cuando haya filtro de fecha activo.
> 3. En `renderTabla()`, para el filtro 'todos' usa `s.base` en vez de `remisiones` directamente, y elimina el bloque de filtro de fechas al final (ya no es necesario).
> 4. En los event listeners de `fecha-desde`, `fecha-hasta` y `btn-limpiar-fechas`, cambia `renderTabla()` por `actualizarUI()`.
...
- [ ] Seleccionar una fecha en "Desde" → los 4 tiles cambian a valores del período
- [ ] El tile "Ganancia Final" también refleja el período filtrado
- [ ] El subtítulo del tile azul muestra "N en el período · M total"
- [ ] Al limpiar fechas → tiles vuelven a los totales globales
- [ ] Los filtros Entregados / Pendientes / Devueltos también respetan las fechas
