...
function renderizarTabla(filas) {
  ...
  filas.forEach((p, i) => {
    ...
    tr.innerHTML = `
      ...
      <td>${p.valor}</td>
      <td>
        <span class="badge-estado ${claseEstado(estado)}" data-key="${p._key}" data-id="${p.id}" style="cursor:pointer">${estado}</span>
      </td>
      <td>
        <div class="td-acciones">
          ...
          <div class="accion-grupo effi-grupo">
            ...
            <button class="btn-accion btn-accion-remarketing" data-id="${p.id}" title="Enviar remarketing por WhatsApp" aria-label="Remarketing WA">
          ...
          <div class="accion-grupo confirmar-grupo">
            <span class="badge-estado ${claseEstado(estado)}" data-key="${p._key}" data-id="${p.id}" style="font-size:0.62rem;padding:0.18rem 0.45rem">${estado}</span>
