...
export async function GET(req: NextRequest) {
  ...
  let propuestasTotal = 0;
  let chatsTotal = 0;

  const { tenants, errores } = await porCadaTenant(async (supabase, tenant) => {
    // ── 1. Traer los mensajes del período (de este tenant) ────────────────────
    ...
    if (error) { console.error(`[Aprendizaje] tenant ${tenant.slug}:`, error.message); return; }
    if (!mensajes || mensajes.length < 10) return;
    ...
    const yaAprendidas = await reglasAprobadas(200, tenant.id);
    ...
    const sistema =
      `Eres el analista de calidad de una tienda que vende por WhatsApp con pago contra entrega en Colombia.\n\n` +
    ...
    try {
      ...
      console.error(`[Aprendizaje] respuesta no válida (tenant ${tenant.slug}):`, e?.message, '| inicio:', crudo.slice(0, 300));
      return;
    }

    if (!Array.isArray(propuestas) || propuestas.length === 0) return;
    ...
    if (insErr) { console.error(`[Aprendizaje] insert tenant ${tenant.slug}:`, insErr.message); return; }
