 ...
 * libres (solo plantillas de pago). Por eso se dispara cuando el último mensaje del
 * CLIENTE lleva entre ~20h y ~23.5h — así el mensaje sí le llega GRATIS.
 *
 * MULTI-TENANT: se corre por cada empresa activa, con SU línea de ventas.
 * Se ejecuta cada ~30 min por cron-job.org.
 */

// Mensajes cortos, con descuento y CTA. Se rota para no sonar igual a todos.
const PROMOS = [
  '¡Hola! 👋 Solo por *HOY* te dejo *$10.000 de descuento* en tu pedido 🔥\n¿Te lo aparto? Dime los datos y lo dejo listo para despacho 🚚',
  '🔥 *Solo hoy*: $10.000 OFF en lo que viste.\nSi confirmas hoy te lo despacho ya 🚚 ¿Lo aprovechas? 😊',
  '¡${nombre}! 🎁 Hoy tienes *$10.000 de descuento* — pero solo por hoy.\n¿Lo aprovechas? Mándame tus datos y cerramos tu pedido 🙌',
];
...
export async function GET(req: NextRequest) {
  ...
  const { tenants, errores } = await porCadaTenant(async (supabase, tenant) => {
    const numeroVentas = tenant.wa_phone_number_id_ventas ?? tenant.wa_phone_number_id;
    if (!numeroVentas) return; // este tenant no tiene línea
    ...
    await conLinea(
      {
        ...
        tenantId: tenant.id,
        phoneIdVentas: tenant.wa_phone_number_id_ventas ?? undefined,
      },
      async () => {
  ...
  return NextResponse.json({ ok: true, enviados, revisados, tenants, errores });
}
