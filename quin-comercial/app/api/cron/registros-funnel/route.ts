 ...
 * Se ejecuta cada 1–2 min por cron. MULTI-TENANT: por cada empresa activa.
 */
const GRACIA_MIN = 5;
...
export async function GET(req: NextRequest) {
  ...
  const { tenants, errores } = await porCadaTenant(async (supabase, tenant) => {
    // El dueño recibe sus ventas en su número vinculado (o respaldo agencia).
    const destinos = await destinosAviso(tenant.id);
    ...
    for (const p of pendientes ?? []) {
      ...
      try {
        ...
        if (esVentaWA) await mandarFichaVentaWA(supabase, p, destinos);
        else           await mandarFichaVenta(supabase, p, tel10, from, destinos);
  ...
  return NextResponse.json({ status: 'ok', enviadas, revisadas, tenants, errores });
}
