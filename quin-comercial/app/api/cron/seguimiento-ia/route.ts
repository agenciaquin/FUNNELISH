...
export async function GET(req: NextRequest) {
  ...
  const { tenants, errores } = await porCadaTenant(async (supabase, tenant) => {
    if (sinTiempo || enviados >= MAX_ENVIOS) return; // ya no hay presupuesto

    // Candidatos de ESTE tenant: chats de ventas activos en las últimas 24h,
    // bot encendido, que aún no llegaron al tope de intentos.
    const { data: convs, error } = await supabase
      ...
      .limit(80);
    if (error) { console.error(`[SeguimientoIA] tenant ${tenant.slug}:`, error.message); return; }
    if (!convs?.length) return;

    // Mensajes salen por la línea de VENTAS de ESTE tenant (o la principal si no hay).
    const lineaVentas = tenant.wa_phone_number_id_ventas ?? tenant.wa_phone_number_id ?? '';
      ...
      async () => {
        for (const c of convs) {
          ...
          const texto = lista
            ...
            .map(m => `${m.role === 'user' ? 'CLIENTE' : m.role === 'agent' ? 'ASESOR' : 'BOT'}: ${m.content.trim()}`)
            .join('\n')
            .slice(-4000);
          if (!texto) { saltados++; continue; }

          const sistema =
            `Eres la asesora de una tienda que vende por WhatsApp con pago contra entrega en Colombia. ` +
