...
export async function GET(req: NextRequest) {
  ...
  const { count: pendientes } = await supabase
    .from('faq_bot').select('*', { count: 'exact', head: true }).eq('tenant_id', tid).eq('estado', 'propuesta');
  const { count: aprobadas } = await supabase
    .from('faq_bot').select('*', { count: 'exact', head: true }).eq('tenant_id', tid).eq('estado', 'aprobada');
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (accion === 'aprobar') {
      ...
      const { error } = await supabase.from('faq_bot').update(cambios).eq('id', id).eq('tenant_id', tid);
      if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    ...
    if (accion === 'borrar-definitivo') {
      if (!id) return NextResponse.json({ error: 'Falta el id.' }, { status: 400 });
      const { error } = await supabase.from('faq_bot').delete().eq('id', id).eq('tenant_id', tid);
      if (error) return NextResponse.json({ error: error.message }, { status: 500 });
      return NextResponse.json({ ok: true });
    }
