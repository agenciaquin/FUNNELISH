...
import { entrarLinea, type BaseLinea } from '@/lib/whatsapp-contexto';
...
function parsePack(productoNombre: string, variantName: string): {
  ...
  // Si cada prenda lleva su propia talla, se muestran todas (ej: "S HOMBRE + M HOMBRE").
  // Si son iguales, se ve una sola.
  const tallasUnicas = [...new Set(tallas.map(t => t.trim()).filter(Boolean))];
  const tallaFinal = tallasUnicas.length ? tallasUnicas.join(' + ') : 'Por confirmar';
  // Emparejar producto + talla cuando hay una talla por prenda (color, talla, color, talla…)
  let productoFinal = colores.length >= 2 ? productos.join(' + ') : productoNombre;
  if (colores.length >= 2 && tallas.length === colores.length) {
    productoFinal = productos.map((prod, i) => `${prod} (${tallas[i].trim()})`).join(' + ');
  }
  return {
...
/**
 * Procesa un pedido y manda la confirmación por WhatsApp.
 * Lo usa el EMBUDO INTERNO (`/api/pedidos`), que le pasa `base` con el tenant y
 * sus credenciales de WhatsApp. Sin `base` funciona en modo single-tenant (env).
 */
export async function procesarPedidoFunnelish(req: NextRequest, base?: BaseLinea) {
  // MULTI-TENANT: fija el tenant y su WhatsApp para TODO el pedido (DB + envíos).
  if (base?.tenantId) {
    entrarLinea({
      phoneId: base.phoneId ?? process.env.WHATSAPP_PHONE_NUMBER_ID ?? '',
      tipo: 'funnel',
  ...
  // La foto que mandó la PÁGINA es la del color/producto EXACTO que el cliente
  // eligió, así que tiene PRIORIDAD sobre la búsqueda en el catálogo (que a veces
  // trae un producto equivocado, ej. otra referencia como "Spiderman"). El
  // catálogo queda solo de respaldo cuando la página no mandó foto.
  if (imagenPagina.startsWith('http')) {
  ...
  // Si esta MISMA referencia ya tenía la confirmación enviada, NO se reenvía
  // (evita el doble mensaje cuando el pedido se procesa dos veces).
  let yaEnviado = false;
  if (referencia) {
    const { data: existing, error: findErr } = await supabase
      .from('clientes_funnelish').select('id, wa_enviado').eq('referencia', referencia).maybeSingle();
    if (findErr) console.error('[Funnelish] buscar referencia error:', findErr.message);

    if (existing?.id) {
      if ((existing as any).wa_enviado === true) yaEnviado = true;
  ...
  // Ya se le había enviado la confirmación a este pedido → no repetir el mensaje.
  if (yaEnviado) {
    console.log(`[Funnelish] referencia ${referencia} ya tenía confirmación enviada → no se reenvía`);
    return NextResponse.json({ success: true, phone: `57${tel10}`, yaEnviado: true });
  }
  ...
  // ── Atribución por embudo ────────────────────────────────────────────────────
  // Guarda en el pedido el embudo (slug) del que entró, para el contador de
  // ventas por embudo del panel. Best-effort: si la columna funnel_slug aún no
  // existe (falta correr la migración), el update simplemente no aplica y el
  // pedido sigue su curso normal sin romperse.
  const funnelSlug = String(body.slug ?? '').trim();
  if (pedidoGuardado && pedidoId && funnelSlug) {
    const { error: slugErr } = await supabase
      .from('clientes_funnelish').update({ funnel_slug: funnelSlug }).eq('id', pedidoId);
    if (slugErr) console.warn('[Funnelish] no se pudo guardar funnel_slug (¿falta migración?):', slugErr.message);
  }
  ...
  if (sent) {
    ...
    // Modo de confirmación del embudo (viene en el pedido):
    //  'solo'  = el bot YA envió la confirmación → se APAGA (bot_enabled=false).
    //            Si el cliente responde, lo atiende una persona. La venta queda
    //            "PENDIENTE POR CONFIRMACIÓN" para revisarla y pasarla a despacho.
    //  'agente'/null = el bot sigue atendiendo y cierra la venta él mismo (por defecto).
    const modoConfirmacion = String(body.modo_confirmacion ?? '').trim().toLowerCase();
    const botActivoTrasPedido = modoConfirmacion !== 'solo';
    const { error: convErr } = await supabase.from('conversations').upsert({
      ...
      unread_count:      1,
      bot_enabled:       botActivoTrasPedido,
      label:             'PENDIENTE POR CONFIRMACIÓN',
    }, { onConflict: 'tenant_id,id' });
    if (convErr) console.error('[Funnelish] upsert conversation error:', convErr.message);
    if (!botActivoTrasPedido) console.log(`[Funnelish] embudo "solo confirmar" → bot APAGADO para ${waPhone} (atiende humano)`);
    ...
    // ── 3b. (Eliminado) Antes se enviaba una SEGUNDA foto por separado para los
    //    packs x2. Ya no: el pack va en UN solo collage dentro de la foto de la
    //    plantilla, así que reenviarla duplicaba la imagen y, como el cliente aún
    //    no ha escrito (ventana de 24h cerrada), además fallaba con error 131047.
    //    Se deja únicamente el mensaje de la plantilla con su foto.
