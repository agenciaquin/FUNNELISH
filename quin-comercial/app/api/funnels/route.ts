...
export async function POST(req: NextRequest) {
  try {
    ...
    if (error && /column .*(pixel_meta_token|pixel_tiktok_token|audio_url|video_url|color|miniatura_url|anuncios|bloques|layout|layout_borrador|modo_publicado|modo_confirmacion|ocultar_boton2|catalogo_id|pie_empresa).* does not exist/i.test(error.message)) {
      const { pixel_meta_token, pixel_tiktok_token, audio_url, video_url, color, miniatura_url, anuncios, bloques, layout, layout_borrador, modo_publicado, modo_confirmacion, ocultar_boton2, catalogo_id, pie_empresa, ...sinNuevas } = fila;
