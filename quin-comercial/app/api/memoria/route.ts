...
export async function GET(req: NextRequest) {
  ...
  const { count: pendientes } = await supabase
    .from('memoria_bot').select('*', { count: 'exact', head: true }).eq('tenant_id', tid).eq('estado', 'propuesta');
  const { count: aprendidas } = await supabase
    .from('memoria_bot').select('*', { count: 'exact', head: true }).eq('tenant_id', tid).eq('estado', 'aprobada');
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (accion === 'aprobar') {
      ...
      const { error } = await supabase.from('memoria_bot').update(cambios).eq('id', id).eq('tenant_id', tid);
      if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    ...
    if (accion === 'descartar') {
      ...
      const { error } = await supabase.from('memoria_bot')
        .update({ estado: 'descartada', aprobada_at: null }).eq('id', id).eq('tenant_id', tid);
      if (error) return NextResponse.json({ error: error.message }, { status: 500 });
      return NextResponse.json({ ok: true });
    }
    ...
    if (accion === 'restaurar') {
      if (!id) return NextResponse.json({ error: 'Falta el id.' }, { status: 400 });
      const { error } = await supabase.from('memoria_bot')
        .update({ estado: 'propuesta' }).eq('id', id).eq('tenant_id', tid);
      if (error) return NextResponse.json({ error: error.message }, { status: 500 });
      return NextResponse.json({ ok: true });
    }
    ...
    if (accion === 'borrar-definitivo') {
      if (!id) return NextResponse.json({ error: 'Falta el id.' }, { status: 400 });
      const { error } = await supabase.from('memoria_bot').delete().eq('id', id).eq('tenant_id', tid);
      if (error) return NextResponse.json({ error: error.message }, { status: 500 });
      return NextResponse.json({ ok: true });
    }
    ...
    if (accion === 'aprobar-todas') {
      const { error } = await supabase.from('memoria_bot')
        .update({ estado: 'aprobada', aprobada_at: ahora }).eq('tenant_id', tid).eq('estado', 'propuesta');
      if (error) return NextResponse.json({ error: error.message }, { status: 500 });
      return NextResponse.json({ ok: true });
    }
