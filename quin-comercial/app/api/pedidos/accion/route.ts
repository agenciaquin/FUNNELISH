...
export async function POST(req: NextRequest) {
  ...
  if (accion === 'eliminar') {
    const { error } = await supabase.from('clientes_funnelish').delete().eq('tenant_id', tid).in('id', ids);
  ...
  if (accion === 'cancelar' || accion === 'restaurar') {
    ...
    const { error } = await supabase.from('clientes_funnelish').update(cambios).eq('tenant_id', tid).in('id', ids);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ ok: true, afectados: ids.length });
  }
