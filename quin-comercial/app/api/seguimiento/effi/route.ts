...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('effi_guias').select('telefono, estado, flete, motivo').eq('tenant_id', tid);
...
export async function POST(req: NextRequest) {
  ...
  // Reemplaza SOLO los teléfonos de este archivo (borra e inserta) DEL TENANT, y
  // conserva los que no vinieron. No depende de una restricción única.
  const { error: delErr } = await supabase
    .from('effi_guias').delete().eq('tenant_id', tid).in('telefono', telefonos);
  if (delErr) return NextResponse.json({ error: `borrar: ${delErr.message}` }, { status: 500 });

  const conTenant = unicas.map(u => ({ ...u, tenant_id: tid }));
  const { error: insErr } = await supabase.from('effi_guias').insert(conTenant);
