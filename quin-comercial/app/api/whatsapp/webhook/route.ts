...
import { bloqueEtiquetas, aplicarMarcadores } from '@/lib/reglas-etiqueta';
import { bloqueCatalogos, leerCatalogos } from '@/lib/bloque-catalogos';
import { cargarComportamiento, REGLAS_FINALES } from '@/lib/quinchat/comportamiento';
...
import { isCompleteAddress, isDirOficina, getAddressQuestion, esPreguntaDireccion, preguntaTalla, preguntaConfirmarTodo } from '@/lib/address';
...
import { preFiltroBot, procesarMarcadorFotos } from '@/lib/disparadores-bot';
...
// Cada mensaje entrante de Meta trae el número de NUESTRA línea en
// value.metadata.display_phone_number. Lo guardamos en el tenant (una vez por
// arranque) para poder mostrarlo en el panel sin pedir permisos extra a Meta.
const numeroTenantGuardado = new Set<string>();
async function guardarNumeroBot(tid: string | null | undefined, value: any) {
  try {
    if (!tid) return;
    const num = String(value?.metadata?.display_phone_number ?? '').replace(/\D/g, '');
    if (!num) return;
    const clave = `${tid}:${num}`;
    if (numeroTenantGuardado.has(clave)) return;
    numeroTenantGuardado.add(clave);
    const admin = createServerSupabaseClient();
    await admin.from('tenants').update({ wa_numero: num }).eq('id', tid).neq('wa_numero', num);
  } catch { /* nunca bloquear el webhook por guardar el número */ }
}
...
/** Envía un aviso operativo (traspaso a humano, etc.). Si se pasa `tid`, va al
 *  número del DUEÑO vinculado (o al respaldo de la agencia). Sin tid, respaldo. */
async function notificarSoporte(texto: string, tid?: string | null) {
  let destinos = SOPORTE;
  if (tid) { try { destinos = await destinosAviso(tid); } catch { /* respaldo */ } }
...
async function marcarHumanoYNotificar(supabase: any, from: string, tid?: string | null) {
  ...
  const nombreCli = conv?.contact_name && conv.contact_name !== 'Desconocido' ? String(conv.contact_name) : '';
  const quien = nombreCli ? `${nombreCli} con el número ${tel}` : `con el número ${tel}`;
  const aviso = `🔔 Hola, el cliente ${quien} necesita de tu asesoría para cerrar la venta. 🙌`;
  await notificarSoporte(aviso, tid);
}
...
export async function procesarEntrada(req: NextRequest, base?: BaseLinea) {
  ...
  // Guarda (1 vez) el número de nuestra línea que viene en el mensaje entrante.
  guardarNumeroBot(tidBot, value).catch(() => {});
  ...
  // Se agregan: el catálogo REAL (para que no invente productos) y las reglas de
  // etiquetas automáticas de la empresa.
  const memoriaBot    = (await cargarComportamiento(supabase))
    + (await bloqueDeMemoria())
    + (await bloqueCatalogos(supabase))
    + (await bloqueEtiquetas(supabase, tidBot))
    + REGLAS_FINALES; // checklist de oro AL FINAL (máxima atención del modelo)
  ...
  for (const msg of value.messages) {
    // Meta normalmente manda el número del cliente en msg.from. Con algunas
    // configuraciones (coexistencia app+API / nueva identidad) el mensaje llega
    // SIN 'from' y el número real viene en contacts[].wa_id. Se toma el primero
    // que exista, priorizando siempre el número de teléfono real.
    const from  = (msg.from
      ?? (value?.contacts?.[0]?.wa_id as string | undefined)
      ?? (msg as any).from_user_id) as string;
    const msgId = msg.id  as string;
    // Aviso de diagnóstico si tuvimos que usar un campo alterno (para ver de dónde salió).
    if (!msg.from) {
      console.warn('[WhatsApp] from alterno usado:', from, '| wa_id:', value?.contacts?.[0]?.wa_id, '| from_user_id:', (msg as any).from_user_id);
    }
    // Red de seguridad: sin remitente no se puede procesar ni responder.
    if (!from) {
      console.warn('[WhatsApp] mensaje sin remitente, ignorado :: contacts=', JSON.stringify(value?.contacts).slice(0, 400));
      continue;
    }
    // TODO el procesamiento de este mensaje va dentro de un try global: si algo
    // suelto falla (un .replace sobre un valor vacío, una columna que falta, etc.)
    // NO tumbamos la función con un 500 (eso hace que Meta reintente y el bot quede
    // mudo). Se registra el error real y se le responde algo al cliente igual.
    try {

    // ── Idempotencia a nivel de PROCESAMIENTO (anti-duplicados de Meta) ───────
    // Meta REINTENTA el webhook si tardamos en responder (fotos + IA pueden pasar
    // los segundos que Meta espera). Sin esto, el MISMO mensaje se procesa 2 veces
    // y el bot manda respuestas repetidas. Marcamos el wamid con un INSERT: si ya
    // existía, es un reintento → no lo reprocesamos. Los upsert de más abajo
    // completan el contenido real del mensaje.
    if (msgId) {
      ...
      if (dupErr) {
        ...
        // Otro tipo de error: no bloqueamos; los upsert siguientes re-guardan el mensaje.
      }
    }
    ...
    if (TIPOS_MEDIA.includes(msg.type)) {
      ...
      let audioComoTexto = false; // true = la nota de voz se tratará como texto del cliente
        ...
        try {
          ...
          if (media) {
            ...
            if (msg.type === 'audio' || msg.type === 'voice') {
              audioTexto = (await transcribirAudio(media.buffer, media.mimeType, tidBot)) ?? '';
            }
          }
        } catch (e) {
      ...
      if (msg.type === 'audio' || msg.type === 'voice') {
        ...
        if (audioTexto) {
          // Se transcribió: se guarda como texto para el historial/panel.
          await supabase.from('messages').upsert({
            id: `${msgId}-txt`, conversation_id: from, content: `🎙️ ${audioTexto}`,
            role: 'user', type: 'text', created_at: new Date().toISOString(),
          }, { onConflict: 'id' });
          if (botActivo) {
            // La nota de voz se TRATA COMO SI EL CLIENTE LA HUBIERA ESCRITO:
            // convertimos el mensaje a texto y dejamos que el flujo normal del bot
            // (catálogo, IA, cierre) la responda de verdad. Así ya no contesta con
            // un "ya la recibí" genérico: entiende y actúa sobre lo que dijo.
            (msg as any).type = 'text';
            (msg as any).text = { body: audioTexto };
            audioComoTexto = true;
          } else {
            continue; // bot apagado en este chat → lo atiende una persona
          }
        } else {
          // No se pudo transcribir: se pide que lo escriba (si el bot está activo).
          if (botActivo) {
            const respuesta =
        ...
        // Si audioComoTexto = true, NO hacemos continue: caemos al flujo de texto.
      }

      // Solo las fotos (o una nota de voz convertida a texto) siguen adelante;
      // los demás archivos (video, documento, sticker) terminan aquí.
    ...
    if (msg.type === 'image') {
      ...
      // Si NO es comprobante (ej. foto de un producto), no se marca abono ni se
      // dice "recibí tu comprobante". El bot MIRA la foto (visión) y responde
      // como vendedor: usa una IA con visión (o la de agencia como respaldo).
      if (!esComprobante) {
        if (botActivo) {
          const caption = ((msg as any).image?.caption ?? '').trim();
          let r = '¡Gracias por la foto! 😊 ¿En qué te ayudo con tu pedido? Si me enviaste tu comprobante de pago, ¿me lo confirmas? 🙌';
          try {
            const { data: histFoto } = await supabase
              .from('messages').select('content, role')
              .eq('conversation_id', from).order('created_at', { ascending: false }).limit(10);
            const histMsgs = [...(histFoto ?? [])].reverse()
              .filter((m: any) => m.content?.trim() && !m.content.startsWith('http'))
              .map((m: any) => ({ role: (m.role === 'user' ? 'user' : 'assistant') as 'user' | 'assistant', content: m.content as string }));
            histMsgs.push({ role: 'user', content: caption || 'Te envié una foto. ¿Qué me puedes decir sobre esto para mi pedido?' });
            const sysFoto = (await cargarPromptEmpresa(supabase)) + ANDAMIAJE_SIN_PEDIDO + memoriaBot;
            const resp = await responderIA(tidBot ?? null, {
              messages: histMsgs,
              systemPrompt: sysFoto,
              conversationId: from,
              imagenes: imgBuffer ? [{ mimeType: imgMime || 'image/jpeg', base64: imgBuffer.toString('base64') }] : undefined,
            });
            if (resp?.message?.trim()) {
              const proc = (await procesarMarcadorFotos(supabase, from, await aplicarMarcadores(supabase, from, resp.message))).trim();
              if (proc) r = proc;
            }
          } catch (e) { console.error('[Foto] respuesta con visión falló:', e); }
    ...
    // ⚠️ FLUJO DE ABONO/PAGO CON DATOS FIJOS DE KLIXMANT (cuentas, Nequi, audios).
    // Es EXCLUSIVO del bot propio de Klixmant. Para el resto de clientes (SaaS) NO
    // debe ejecutarse: cada negocio tiene sus propias cuentas en su Entrenamiento y
    // el bot responde los pagos desde ahí. Si esto corriera para todos, se filtraría
    // el Nequi/Bancolombia de Klixmant a otros bots (justo el bug que arreglamos).
    const flujoAbonoKlixmant = tidBot === 'klixmant';
    ...
    if (flujoAbonoKlixmant && abonoObjection) {
    ...
    if (flujoAbonoKlixmant && textLower.includes('nequi')) {
      const MSG_NEQUI =
    ...
    if (flujoAbonoKlixmant && (abonoAccountReq || aceptoDatosAbono)) {
    ...
    if (flujoAbonoKlixmant && (isOficina || isMunicipio)) {
    ...
    // Frases EXPLÍCITAS de cancelar. NADA de "no me interesa" suelto: se colaba en
    // frases como "el catálogo no me interesa" y cancelaba sin que el cliente lo pidiera.
    const cancelaPedido = ['cancela mi pedido', 'cancelar mi pedido', 'cancela el pedido',
      'cancelar el pedido', 'cancelo el pedido', 'cancelo mi pedido', 'anula mi pedido',
      'anular mi pedido', 'anula el pedido', 'anular el pedido', 'no quiero el pedido',
      'no quiero mi pedido', 'ya no quiero el pedido', 'no deseo el pedido',
      'no voy a comprar', 'no lo voy a comprar'].some(w => textLower.includes(w));
    ...
    // Cancelar/programar SOLO tiene sentido si el cliente TIENE un pedido activo.
    // Si solo está conversando (sin pedido), NO se cancela nada: lo maneja la IA.
    let tienePedidoActivo = false;
    if (cancelaPedido || programaPedido) {
      const telAct = from.replace(/^57/, '').slice(-10);
      const { data: pedActivo } = await supabase
        .from('clientes_funnelish').select('id')
        .eq('telefono', telAct).eq('confirmado', false)
        .not('estado', 'in', '("cancelado","duplicado")')
        .limit(1).maybeSingle();
      tienePedidoActivo = !!pedActivo?.id;
    }

    if (cancelaPedido && tienePedidoActivo) {
    ...
    if (programaPedido && tienePedidoActivo) {
    ...
    const isConfirmo = ['CONFIRMO', '✅ CONFIRMO', 'CONFIRMO ✅', 'SI CONFIRMO', 'SÍ CONFIRMO', 'CONFIRMO PEDIDO', '✅ CONFIRMO PEDIDO']
      .includes(textClean)
    ...
    // ── Botón "MODIFICAR DATOS" de la plantilla de confirmación ──────────────
    // El cliente tocó el botón para corregir un dato: NO se confirma, se le pide
    // que escriba la corrección y el bot la toma. Si el bot está apagado en este
    // chat (embudo "solo confirmar"), no respondemos: lo atiende una persona.
    const isModificar = !isConfirmo && ['MODIFICAR DATOS', 'MODIFICAR', 'CORREGIR DATOS', 'CORREGIR', 'MODIFICAR DATO']
      .includes(textClean);
    if (isModificar) {
      if (botEnabled) {
        const msgMod = '¡Claro! 📝 Cuéntame qué dato deseas corregir (dirección, talla, color, teléfono, etc.) y escríbelo aquí para actualizarlo. 😊';
        const w = await sendTextMessage(from, msgMod);
        await saveAndSend(supabase, from, msgMod, 'text', w);
      }
      continue;
    }
    ...
    if (!pendingPedido) {
      ...
      if (confirmedPedido) {
        ...
        if (wantsOtherProduct) {
          ...
          await marcarHumanoYNotificar(supabase, from, tidBot);
          continue;
        }
        ...
        try {
          const resp = await responderIA(tidBot ?? null, { messages: histMsgs, systemPrompt: sysConf, conversationId: from });
          ...
          const finalConf = await procesarMarcadorFotos(supabase, from, await aplicarMarcadores(supabase, from, visible || resp.message));
          if (finalConf.trim()) {
            const wamid = await sendTextMessage(from, finalConf);
            if (wamid) await saveAndSend(supabase, from, finalConf, 'text', wamid);
          }
        } catch { /* ignorar error */ }
        continue;
      }
      ...
      try {
        ...
        const pideAsesorNP = /\[\[\s*ASESOR\s*\]\]/i.test(resp.message ?? '');
        let finalNP = await procesarMarcadorFotos(supabase, from, await aplicarMarcadores(supabase, from, resp.message));
        if (pideAsesorNP && !finalNP.trim()) {
          finalNP = 'Permíteme un momento y te ayuda un asesor para darte esa información y cerrar tu pedido 😊';
        }
        if (finalNP.trim()) {
          const wamid = await sendTextMessage(from, finalNP);
          if (wamid) await saveAndSend(supabase, from, finalNP, 'text', wamid);
        }
        if (pideAsesorNP) await marcarHumanoYNotificar(supabase, from, tidBot);
      } catch { /* ignorar */ }
      continue;
    }

    // ── Solicitud de catálogo → enviar las CATEGORÍAS REALES ─────────────────
    // El cliente pide ver el catálogo/qué hay. En vez de pasarlo siempre a un
    // humano (y en vez de que el bot invente modelos), le mostramos las
    // categorías que la tienda creó de verdad. Solo si NO hay ningún catálogo
    // cargado se pasa a un asesor.
    const catalogWords = ['catalogo', 'catálogo', 'más productos', 'mas productos',
      ...
      'ver todo', 'ver todos', 'más modelos', 'mas modelos', 'que categorias', 'qué categorías',
      'que venden', 'qué venden', 'que manejan', 'qué manejan'];
    if (catalogWords.some(w => textLower.includes(w))) {
      const cats = await leerCatalogos(supabase);
      if (cats.length) {
        const lista = cats.map(c => `• ${c.familia}`).join('\n');
        const msjCat = cats.length === 1
          ? `Claro 😊 Manejamos ${cats[0].familia}. ¿Te muestro lo que hay?`
          : `¡Claro! 😊 Estas son nuestras categorías:\n${lista}\n\n¿Cuál te gustaría ver?`;
        const wamid = await sendTextMessage(from, msjCat);
        await saveAndSend(supabase, from, msjCat, 'text', wamid);
        continue;
      }
      // Sin catálogos cargados: recién ahí se pasa a un asesor.
      ...
      if (!isCompleteAddress(combinada) && !respondeDireccion) {
        ...
        if (intentosDir >= 3) {
          ...
          if (intentosDir >= 2) {
            ...
            msjDir = getAddressQuestion(combinada, pendingPedido.nombre) ?? '📍 Necesito la dirección completa. Por ejemplo: *Calle 15 # 20-30 Barrio Los Pinos*.';
          }
    ...
    let stillMissingDir     = !isCompleteAddress(currentDireccion) && !isDirOficinaType && !direccionOk;

    // ── ANTI-BUCLE de dirección incompleta (vereda / urbanización sin número) ──
    // Muchas direcciones rurales NO tienen número: insistir era lo que enloquecía
    // al cliente. Regla humana: se pregunta UNA sola vez. Si ya preguntamos antes
    // (pidio_confirmar) o el cliente ya está afirmando, se da la dirección por buena
    // y se cierra; si es la primera vez, se pregunta con un mensaje personalizado.
    {
      const dirTxt = String(currentDireccion ?? '').trim().replace(/—/g, '');
      const dirTieneTexto = dirTxt.length >= 5;
      const soloFaltaDir  = stillMissingDir && !stillMissingTalla && !isDirOficinaType;
      if (soloFaltaDir && dirTieneTexto && !respondeDireccion) {
        const nombreCorto = String(pendingPedido.nombre ?? '').split(' ')[0] || '';
        const niegaAhora = /\b(no|nop|nel|cambi|corrig|corrij|error|equivoc|est[áa]\s*mal|falta|faltan|otra\s*direcci)\b/i.test(textLower);
        const afirmaAhora =
          /\b(si|s[ií]|claro|correcto|correcta|correctos|exacto|listo|dale|ok|okay|perfecto|confirmo|confirmado|confirmar|as[ií]\s*es|eso\s*es|de\s*una|h[aá]gale|todo\s*bien|todo\s*correcto|est[aá]\s*bien|est[aá]\s*correcto)\b/i.test(textLower)
          && !niegaAhora;
        const yaPreguntamos = pendingPedido.pidio_confirmar === true;

        if (!niegaAhora && (yaPreguntamos || afirmaAhora)) {
          // Ya le preguntamos antes (o el cliente afirma) → dar por buena y cerrar.
          // No se repite la pregunta: era justo lo que hartaba al cliente.
          await supabase.from('clientes_funnelish')
            .update({ direccion_ok: true }).eq('id', pendingPedido.id);
          direccionOk = true;
          stillMissingDir = false;
        } else if (!yaPreguntamos && !niegaAhora) {
          // Primera vez: preguntar UNA sola vez, personalizado y variado. Se marca
          // pidio_confirmar para que, al reconfirmar, la venta cierre sin repetir.
          await supabase.from('clientes_funnelish')
            .update({ pidio_confirmar: true }).eq('id', pendingPedido.id);
          const q = getAddressQuestion(currentDireccion, nombreCorto)
            ?? '📍 ¿Me confirmas que tu dirección ya quedó completa? Si es vereda o urbanización sin número, mándame un punto de referencia 🚚';
          const w = await sendTextMessage(from, q);
          await saveAndSend(supabase, from, q, 'text', w);
          continue;
        }
        // Si el cliente NIEGA/corrige, no se cierra ni se repite: sigue el flujo
        // normal para que dé la dirección corregida.
      }
    }
    ...
    if (clientGaveTalla && clientGaveDireccion) {
      ...
      if (isDirOficinaType) {
        ...
        const addrQTalla = getAddressQuestion(currentDireccion, nombrePedido) ?? '📍 Para completar el pedido necesito tu dirección de domicilio completa (ej: Calle 15 # 20-30, Barrio). ¿Cuál es?';
    ...
    let botReply: string;
    let pideAsesor = false;
    try {
      const resp = await responderIA(tidBot ?? null, { messages: chatHistory, systemPrompt: sysPrompt, conversationId: from });
      pideAsesor = /\[\[\s*ASESOR\s*\]\]/i.test(resp.message ?? '');
      botReply = await procesarMarcadorFotos(supabase, from, await aplicarMarcadores(supabase, from, resp.message));
    } catch { continue; }

    // Si el bot no sabe/no tiene la info, pidió pasar a un asesor: se le manda al
    // cliente un mensaje amable, se APAGA el bot y se avisa al número principal.
    if (pideAsesor && !botReply.trim()) {
      botReply = 'Permíteme un momento y te ayuda un asesor para darte esa información y cerrar tu pedido 😊';
    }
    // Si el mensaje quedó vacío tras enviar las fotos, no mandamos texto en blanco.
    if (!botReply.trim()) continue;
    ...
    if (pideAsesor) await marcarHumanoYNotificar(supabase, from, tidBot);
    } catch (errMsg) {
      // Se ve completo en los logs de Vercel para ubicar la causa exacta.
      console.error('[WhatsApp] ERROR procesando mensaje de', from, '::', (errMsg as any)?.stack || String(errMsg));
      // No dejar mudo al cliente: se le responde con la IA (o un saludo de respaldo).
      try {
        const sysFb = (await cargarPromptEmpresa(supabase)) + ANDAMIAJE_SIN_PEDIDO + memoriaBot;
        const r = await responderIA(tidBot ?? null, {
          messages: [{ role: 'user', content: (msg as any)?.text?.body || 'Hola' }],
          systemPrompt: sysFb,
          conversationId: from,
        });
        const rFb = await procesarMarcadorFotos(supabase, from, await aplicarMarcadores(supabase, from, r.message));
        if (rFb.trim()) {
          const w = await sendTextMessage(from, rFb);
          if (w) await saveAndSend(supabase, from, rFb, 'text', w);
        }
      } catch { /* si el respaldo también falla, al menos no truena el webhook */ }
    }
  }
