...
import { numeroDelBotTenant } from '@/lib/whatsapp';
...
export default async function PaginaGracias({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Número del bot de ESTA tienda (según el tenant del embudo), nunca el de otra.
  const whatsappBot = await numeroDelBotTenant((f as any).tenant_id);

  return (
    <main className="min-h-screen bg-white max-w-lg mx-auto px-5 py-8 text-center">
      <FunnelTracker slug={slug} paso="compra" />
