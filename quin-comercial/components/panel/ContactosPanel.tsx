...
function labelColor(name: string, etiquetasMap: Record<string, string>): string {
  return etiquetasMap[name] ?? DEFAULT_COLORS[name] ?? '#00A89D';
}
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden bg-[#FAF9F6]">
      ...
      <div className="flex items-center justify-between px-8 py-5 border-b border-[#E8E8E8]">
        <h1 className="text-xl font-bold text-[#0D0D0D] tracking-tight">Contactos</h1>
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <input
              ...
              className="w-60 bg-white border border-[#E8E8E8] text-[#0D0D0D] placeholder:text-[#B5B5B5] text-sm rounded-lg px-3 py-2 pl-9 focus:outline-none focus:border-[#00A89D]/50"
            />
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg border text-sm transition-all ${
                filterLabel
                  ...
                  : 'bg-white border-[#E8E8E8] text-[#6B6B6B] hover:text-[#0D0D0D]'
              }`}
            >
              ▾
            </button>
            {showFilter && (
              <div className="absolute right-0 top-10 z-30 bg-white border border-[#E8E8E8] rounded-xl shadow-2xl py-1 min-w-[220px]">
                <button
                  onClick={() => { setFilterLabel(''); setShowFilter(false); }}
                  className={`w-full text-left px-4 py-2.5 text-sm font-medium ${
                    !filterLabel ? 'bg-[#00A89D] text-white' : 'text-[#6B6B6B] hover:bg-[#F5F5F5]'
                  }`}
                >
                  Todos
                </button>
                {allLabelNames.map(l => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 text-sm flex items-center gap-2 ${
                      filterLabel === l ? 'bg-[#00A89D]/10 text-[#00A89D]' : 'text-[#6B6B6B] hover:bg-[#F5F5F5]'
                    }`}
                  >
          ...
          <button
            onClick={openAdd}
            className="flex items-center gap-1.5 px-4 py-2 bg-[#00A89D] hover:bg-[#00847A] text-white font-bold text-sm rounded-lg transition-all"
          >
            + Agregar
          </button>
        </div>
      ...
      <div className="flex-1 overflow-auto" onClick={() => showFilter && setShowFilter(false)}>
        {loading ? (
          <div className="flex items-center justify-center h-40">
            <p className="text-[#9A9A9A] text-sm">Cargando contactos…</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#E8E8E8]">
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold text-[#9A9A9A] uppercase tracking-wider">Teléfono</th>
                <th className="px-6 py-3 text-left text-xs font-semibold text-[#9A9A9A] uppercase tracking-wider">País</th>
                <th className="px-6 py-3 text-left text-xs font-semibold text-[#9A9A9A] uppercase tracking-wider">Nombre</th>
                <th className="px-6 py-3 text-left text-xs font-semibold text-[#9A9A9A] uppercase tracking-wider">Etiqueta</th>
                <th className="px-6 py-3 text-left text-xs font-semibold text-[#9A9A9A] uppercase tracking-wider">Creado</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-16 text-center text-[#9A9A9A]">
                    {search || filterLabel
                ...
                <tr
                  key={c.id}
                  onClick={() => openEdit(c)}
                  className="border-b border-[#E8E8E8] hover:bg-[#F5F5F5] transition-colors cursor-pointer"
                >
                  <td className="px-6 py-3 text-[#9A9A9A] text-base">−</td>
                  <td className="px-6 py-3 text-[#3A3A3A]">{c.telefono_numero}</td>
                  <td className="px-6 py-3 text-[#6B6B6B]">{c.pais}</td>
                  <td className="px-6 py-3">
                    <span className={c.nombre ? 'text-[#00A89D]' : 'text-[#9A9A9A]'}>
                      {c.nombre || '—'}
                    </span>
                  </td>
                  <td className="px-6 py-3">
                    {c.etiqueta && (
                      <span className="flex items-center gap-1.5">
                        ...
                        <span className="text-[#3A3A3A]">{c.etiqueta}</span>
                      </span>
                    )}
                  </td>
                  <td className="px-6 py-3 text-[#9A9A9A] text-xs">{fmtDate(c.created_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
