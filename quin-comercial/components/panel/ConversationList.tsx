...
const FILTER_TABS: { key: string; label: string; color: string; test: (c: Conversation) => boolean }[] = [
  ...
  { key: 'funnel',    label: 'Funnel',                   color: '#6366F1', test: c => (c as any).linea !== 'ventas' },
  { key: 'wa',        label: 'WhatsApp',                 color: '#25D366', test: c => (c as any).linea === 'ventas' },
...
interface Props {
  ...
  refreshing?: boolean;
  onSoporte?: () => void;
  duenoNumero?: string;                         // número del dueño (avisos)
  onVincularDueno?: (num: string) => void;      // guardar/cambiar ese número
}

export default function ConversationList({ conversations, selectedId, onSelect, onDelete, loading, onMenuClick, onRefresh, refreshing, onSoporte, duenoNumero, onVincularDueno }: Props) {
  const [editandoDueno, setEditandoDueno] = useState(false);
  const [numDueno, setNumDueno]           = useState('');
  ...
  return (
    <div className={`${selectedId ? 'hidden md:flex' : 'flex'} w-full md:w-[350px] flex-col min-h-0 h-full bg-[#FAF9F6] border-r border-[#E8E8E8] shrink-0`}>
      ...
      {/* ── Mi WhatsApp: número del dueño donde llegan ventas y traspasos ── */}
      {onVincularDueno && (
        <div className="px-3 pt-2 pb-1">
          {!editandoDueno ? (
            <div className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl bg-[#00A89D]/10 border border-[#00A89D]/30 text-[#00847A]">
              <span className="w-7 h-7 rounded-full bg-[#00A89D] text-white flex items-center justify-center text-sm shrink-0">📲</span>
              <div className="min-w-0 text-left flex-1">
                <div className="text-xs font-bold leading-tight">Mi WhatsApp (avisos)</div>
                <div className="text-[10px] text-[#6B6B6B] leading-tight truncate">
                  {duenoNumero ? `+${duenoNumero} · ventas y traspasos` : 'Sin vincular — el bot no sabe a quién avisar'}
                </div>
              </div>
              <button
                onClick={() => { setNumDueno(duenoNumero ?? ''); setEditandoDueno(true); }}
                className="shrink-0 text-[10px] px-2 py-1 rounded-lg bg-white border border-[#00A89D]/40 font-semibold hover:bg-[#00A89D]/10"
              >
                {duenoNumero ? 'Cambiar' : 'Vincular'}
              </button>
            </div>
          ) : (
            <div className="w-full px-3 py-2 rounded-xl bg-white border border-[#00A89D]/40 flex flex-col gap-2">
              <div className="text-[10px] text-[#6B6B6B]">Número de WhatsApp del dueño (con indicativo, ej: 57 300 123 4567)</div>
              <input
                autoFocus
                value={numDueno}
                onChange={e => setNumDueno(e.target.value)}
                onKeyDown={e => { if (e.key === 'Enter') { onVincularDueno(numDueno.replace(/\D/g, '')); setEditandoDueno(false); } }}
                placeholder="573001234567"
                className="w-full border border-[#E8E8E8] rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:border-[#00A89D]"
              />
              <div className="flex gap-2">
                <button
                  onClick={() => { onVincularDueno(numDueno.replace(/\D/g, '')); setEditandoDueno(false); }}
                  className="flex-1 py-1.5 rounded-lg bg-[#00A89D] text-white text-xs font-bold hover:bg-[#007A72]"
                >Guardar</button>
                <button
                  onClick={() => setEditandoDueno(false)}
                  className="px-3 py-1.5 rounded-lg border border-[#E8E8E8] text-[#6B6B6B] text-xs"
                >Cancelar</button>
              </div>
            </div>
          )}
      ...
      <div className="px-3 py-2 border-b border-[#E8E8E8] flex items-center gap-1.5 flex-wrap">
        ...
        {/* (El filtro "No leído" vive abajo en las pestañas de etiquetas) */}
