...
export default function DisparadoresPanel() {
  ...
  if (view === 'list') return (
    <div className="flex-1 flex flex-col min-w-0 bg-[#FAF9F6] overflow-hidden">
      <div className="flex items-center justify-between px-6 py-4 border-b border-[#E8E8E8] bg-white shrink-0">
        <div>
          <h1 className="text-[#0D0D0D] font-bold text-lg">Disparadores</h1>
          <p className="text-xs text-[#9A9A9A] mt-0.5">Automatizaciones basadas en condiciones</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar..."
              className="bg-white border border-[#E8E8E8] rounded-lg pl-8 pr-3 py-2 text-xs text-[#0D0D0D] placeholder-[#B5B5B5] focus:outline-none focus:border-[#00A89D] w-48" />
          </div>
          <button onClick={openNew} className="w-8 h-8 rounded-lg bg-[#00A89D] text-white font-bold text-lg flex items-center justify-center hover:bg-[#00847A] active:scale-95 transition-all">+</button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-[#FAF9F6] border-b border-[#E8E8E8]">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-[#9A9A9A] uppercase tracking-wider w-28">Condiciones</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-[#9A9A9A] uppercase tracking-wider w-24">Acciones</th>
            ...
            <tbody>
              {filtered.map(d => (
                <tr key={d.id} className="border-b border-[#F0F0F0] hover:bg-[#F5F5F5] cursor-pointer" onClick={() => openEdit(d)}>
                  <td className="px-4 py-3 text-[#9A9A9A]">✏️</td>
                  <td className="px-4 py-3 text-[#0D0D0D] font-medium">{d.nombre}</td>
                  <td className="px-4 py-3">
                    <span className="text-xs px-2 py-0.5 rounded-full bg-[#00A89D]/10 text-[#00A89D] border border-[#00A89D]/20">{d.tipo}</span>
                  </td>
                  <td className="px-4 py-3 text-[#9A9A9A] text-xs">{(Array.isArray(d.condiciones) ? d.condiciones : []).length}</td>
                  <td className="px-4 py-3 text-[#00A89D] text-xs font-medium">{(Array.isArray(d.acciones) ? d.acciones : []).length}</td>
                  <td className="px-4 py-3 text-[#9A9A9A] text-xs">{formatFecha(d.created_at)}</td>
                  <td className="px-4 py-3" onClick={e => toggleActivo(d, e)}>
                    <div className={`w-9 h-5 rounded-full relative transition-all ${d.activo ? 'bg-[#00A89D]' : 'bg-[#D1D1D1]'}`}>
  ...
  // ── EDIT VIEW ─────────────────────────────────────────────────────────────
  return (
    <div className="flex-1 flex min-w-0 overflow-hidden bg-[#FAF9F6] relative">

      {/* Plantilla selector modal */}
      {modal.open && (
        <div className="absolute inset-0 z-50 bg-black/70 flex items-center justify-center">
          <div className="bg-white border border-[#E8E8E8] rounded-2xl w-96 max-h-[500px] flex flex-col shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#E8E8E8]">
              <h3 className="text-[#0D0D0D] font-semibold text-sm">Seleccionar plantilla</h3>
              <div className="flex items-center gap-3">
                <select className="bg-[#F5F5F5] border border-[#E8E8E8] rounded-lg px-2 py-1 text-xs text-[#0D0D0D] focus:outline-none">
                  <option>Todos</option>
                </select>
                <button onClick={() => setModal(m => ({ ...m, open: false }))} className="text-[#9A9A9A] hover:text-[#0D0D0D] text-xl leading-none">×</button>
              </div>
            </div>
            <div className="px-4 py-3 border-b border-[#E8E8E8]">
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#9A9A9A] text-xs">🔍</span>
                <input value={modal.search} onChange={e => setModal(m => ({ ...m, search: e.target.value }))}
                  placeholder="Buscar..." autoFocus
                  className="w-full bg-[#F5F5F5] border border-[#E8E8E8] rounded-lg pl-8 pr-3 py-2 text-xs text-[#0D0D0D] placeholder-[#B5B5B5] focus:outline-none focus:border-[#00A89D]" />
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-2">
              {filteredModal.length === 0 ? (
                <div className="text-center py-8 text-[#9A9A9A] text-xs">Sin plantillas</div>
              ) : filteredModal.map(p => (
                <button key={p.id} onClick={() => selectPlantilla(p)}
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-[#F5F5F5] transition-all text-left">
                  {p.imagen_url ? (
                  ...
                  <span className="text-[#0D0D0D] text-xs font-medium flex-1">{p.nombre}</span>
                  <span className="text-[10px] text-[#9A9A9A]">1 💬</span>
                </button>
              ))}
            </div>
            <div className="px-4 py-3 border-t border-[#E8E8E8]">
              <button onClick={() => setModal(m => ({ ...m, open: false }))}
                className="w-full py-2 rounded-lg bg-[#00A89D] text-white text-xs font-semibold hover:bg-[#00847A] transition-all">
                Cerrar
              </button>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col border-r border-[#E8E8E8] min-w-0 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#E8E8E8] bg-white shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={backToList} className="text-[#9A9A9A] hover:text-[#0D0D0D] transition-colors text-sm">← Volver</button>
            <h2 className="text-[#00A89D] font-semibold text-sm">Disparador</h2>
          </div>
          <div className="flex gap-2">
            {!isNew && (
              <button onClick={deleteD} className="px-3 py-1.5 rounded-lg text-xs text-red-600 border border-red-200 hover:bg-red-50 transition-all">Eliminar</button>
            )}
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-5">
          <div>
            <label className="text-xs text-[#6B6B6B] mb-1.5 block">Nombre</label>
            <input value={current.nombre} onChange={e => setCurrent(p => ({ ...p, nombre: e.target.value }))}
              placeholder="Identificador del disparador"
              className="w-full bg-white border border-[#E8E8E8] rounded-lg px-3 py-2.5 text-sm text-[#0D0D0D] placeholder-[#B5B5B5] focus:outline-none focus:border-[#00A89D]" />
          </div>

          <div>
            <label className="text-xs text-[#6B6B6B] mb-1.5 block">Seleccione el tipo de disparador</label>
            <select value={current.tipo} onChange={e => setCurrent(p => ({ ...p, tipo: e.target.value }))}
              className="w-full bg-white border border-[#E8E8E8] rounded-lg px-3 py-2.5 text-sm text-[#0D0D0D] focus:outline-none focus:border-[#00A89D]">
          ...
          {current.tipo === 'Lógica' && (
            <>
              <div>
                <label className="text-xs text-[#6B6B6B] mb-1.5 block">🔧 Lógica</label>
                <textarea value={current.logica} onChange={e => setCurrent(p => ({ ...p, logica: e.target.value }))}
                  ...
                  className="w-full bg-white border border-[#E8E8E8] rounded-lg px-3 py-2.5 text-sm text-[#0D0D0D] placeholder-[#B5B5B5] focus:outline-none focus:border-[#00A89D] resize-none" />
                <div className="flex items-center justify-between mt-1">
                  <p className="text-[10px] text-[#9A9A9A]">Lógica que usa el asistente para validar los mensajes nuevos</p>
                  <span className="text-[10px] text-[#9A9A9A]">{current.logica.length}/400</span>
                </div>
              </div>

              <div className="flex items-center justify-between bg-white border border-[#E8E8E8] rounded-xl px-4 py-3">
                <span className="text-sm text-[#0D0D0D]">Enviar mensajes generado por el asistente</span>
                <div className="flex items-center gap-3">
                  {current.enviar_asistente && (
                    <div>
                      <label className="text-[10px] text-[#9A9A9A] block mb-1">Tipo de mensajes</label>
                      <select value={current.tipo_mensajes} onChange={e => setCurrent(p => ({ ...p, tipo_mensajes: e.target.value }))}
                        className="bg-[#F5F5F5] border border-[#E8E8E8] rounded-lg px-2 py-1.5 text-xs text-[#0D0D0D] focus:outline-none">
                        <option>Auto</option>
                        <option>Texto</option>
                        <option>Audio</option>
                      </select>
                  ...
                  <button onClick={() => setCurrent(p => ({ ...p, enviar_asistente: !p.enviar_asistente }))}
                    className={`w-10 h-6 rounded-full relative transition-all shrink-0 ${current.enviar_asistente ? 'bg-[#00A89D]' : 'bg-[#D1D1D1]'}`}>
      ...
      <div className="w-[420px] shrink-0 flex flex-col bg-[#FAFAFA]">
        <div className="flex border-b border-[#E8E8E8] shrink-0">
          <button onClick={() => setTabRight('condiciones')}
            className={`flex-1 py-3.5 text-xs font-semibold transition-all ${tabRight === 'condiciones' ? 'text-red-600 border-b-2 border-red-600' : 'text-[#9A9A9A] hover:text-[#6B6B6B]'}`}>
            Condiciones
          </button>
          <button onClick={() => setTabRight('acciones')}
            className={`flex-1 py-3.5 text-xs font-semibold transition-all ${tabRight === 'acciones' ? 'text-[#00A89D] border-b-2 border-[#00A89D]' : 'text-[#9A9A9A] hover:text-[#6B6B6B]'}`}>
            Acciones
          </button>
        </div>
          ...
          {tabRight === 'condiciones' && (
            <>
              {current.condiciones.map(c => (
                <div key={c._id} className="bg-white border border-[#E8E8E8] rounded-xl p-3.5 flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-[#9A9A9A] font-semibold uppercase tracking-wider">
                  ...
                  {c.tipo === 'plantilla_enviada' && (
                    <div className="flex items-end gap-2">
                      <div className="shrink-0">
                        <label className="text-[10px] text-[#9A9A9A] block mb-1">Sí se ha:</label>
                      ...
                      <div className="flex-1 min-w-0">
                        <label className="text-[10px] text-[#9A9A9A] block mb-1">La plantilla:</label>
                        <button onClick={() => openModal('cond', c._id)}
                          className="w-full flex items-center justify-between bg-[#F5F5F5] border border-[#E8E8E8] rounded-lg px-3 py-1.5 text-xs hover:border-[#00A89D]/30 transition-all">
                          <span className={c.plantilla_nombre ? 'text-[#0D0D0D]' : 'text-[#9A9A9A]'}>{c.plantilla_nombre ?? 'No seleccionado aún'}</span>
                  ...
                  {c.tipo === 'palabras' && (
                    <div>
                      <label className="text-[10px] text-[#9A9A9A] block mb-1">Palabras clave (separadas por coma):</label>
                      <input value={c.valor ?? ''} onChange={e => updateCond(c._id, { valor: e.target.value })}
                        placeholder="precio, catálogo, quiero..."
                        className="w-full bg-[#F5F5F5] border border-[#E8E8E8] rounded-lg px-3 py-1.5 text-xs text-[#0D0D0D] placeholder-[#B5B5B5] focus:outline-none focus:border-[#00A89D]" />
                    </div>
                  )}

                  {(c.tipo === 'ultimo_mensaje' || c.tipo === 'cantidad_mensajes' || c.tipo === 'etiqueta' || c.tipo === 'horario' || c.tipo === 'pais') && (
                    <div>
                      <label className="text-[10px] text-[#9A9A9A] block mb-1">Valor:</label>
                      <input value={c.valor ?? ''} onChange={e => updateCond(c._id, { valor: e.target.value })}
              ...
              <div className="relative">
                <button onClick={() => { setShowCondDD(v => !v); setShowAccDD(false); }}
                  className={`w-full py-2.5 rounded-lg border text-xs font-semibold transition-all ${showCondDD ? 'bg-red-50 border-red-300 text-red-600' : 'border-red-200 text-red-600 hover:bg-red-50'}`}>
                  Agregar condición
                </button>
                {showCondDD && (
                  <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-[#E8E8E8] rounded-xl shadow-2xl z-10 overflow-hidden">
                    {TIPOS_CONDICION.map(t => (
                      <button key={t.key} onClick={() => addCondicion(t.key)}
                        className="w-full text-left px-4 py-2.5 text-xs text-[#0D0D0D] hover:bg-[#F5F5F5] transition-all">
                        {t.label}
                      </button>
                    ))}
                  </div>
          ...
          {tabRight === 'acciones' && (
            <>
              {current.acciones.map(a => (
                <div key={a._id} className="bg-white border border-[#E8E8E8] rounded-xl p-3.5 flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-[#9A9A9A] font-semibold uppercase tracking-wider">
                  ...
                  {a.tipo === 'enviar_plantilla' && (
                    <button onClick={() => openModal('acc', a._id)}
                      className="w-full flex items-center justify-between bg-[#F5F5F5] border border-[#E8E8E8] rounded-lg px-3 py-2 text-xs hover:border-[#00A89D]/30 transition-all">
                      <span className={a.plantilla_nombre ? 'text-[#0D0D0D]' : 'text-[#9A9A9A]'}>{a.plantilla_nombre ?? 'No seleccionado aún'}</span>
                      <span className="text-[#9A9A9A]">⬆</span>
                    </button>
                  )}
              ...
              <div className="relative">
                <button onClick={() => { setShowAccDD(v => !v); setShowCondDD(false); }}
                  className={`w-full py-2.5 rounded-lg border text-xs font-semibold transition-all ${showAccDD ? 'bg-[#00A89D]/10 border-[#00A89D]/40 text-[#00A89D]' : 'border-[#00A89D]/25 text-[#00A89D] hover:bg-[#00A89D]/10'}`}>
                  Agregar acción
                </button>
                {showAccDD && (
                  <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-[#E8E8E8] rounded-xl shadow-2xl z-10 overflow-hidden">
