...
import { initBrowserSupabase } from '@/lib/supabase';
...
const AjustesWaPanel     = dynamic(() => import('./AjustesWhatsAppPanel'), { ssr: false });
const EmpresasPanel      = dynamic(() => import('./EmpresasPanel'), { ssr: false });
const PlantillasEmbudoPanel = dynamic(() => import('./PlantillasEmbudoPanel'), { ssr: false });
const AprendizajeQuinoPanel = dynamic(() => import('./AprendizajeQuinoPanel'), { ssr: false });
const AsistenteConexion = dynamic(() => import('./AsistenteConexion'), { ssr: false });
const RecargaPanel = dynamic(() => import('./RecargaPanel'), { ssr: false });
const IntegrarIaPanel = dynamic(() => import('./IntegrarIaPanel'), { ssr: false });
const PlantillasConocimientoPanel = dynamic(() => import('./PlantillasConocimientoPanel'), { ssr: false });
const AsistenteBotPanel = dynamic(() => import('./AsistenteBotPanel'), { ssr: false });
const EntrenarBotPanel  = dynamic(() => import('./EntrenarBotPanel'),  { ssr: false });
...
const DominioPanel       = dynamic(() => import('./DominioPanel'),        { ssr: false });
const CarritosAbandonados = dynamic(() => import('./CarritosAbandonados'), { ssr: false });
const GuiasEffiPanel     = dynamic(() => import('./GuiasEffiPanel'),      { ssr: false });
...
export default function WhatsAppPanel({ userName, sbToken, rol }: Props) {
  ...
  const [duenoNumero, setDuenoNumero]     = useState('');

  // Carga el número del dueño (avisos) para mostrarlo en la lista de chats.
  useEffect(() => {
    fetch('/api/tenant/whatsapp')
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (d?.wa_numero_dueno) setDuenoNumero(String(d.wa_numero_dueno)); })
      .catch(() => {});
  }, []);

  async function guardarDueno(num: string) {
    setDuenoNumero(num); // optimista
    try {
      await fetch('/api/tenant/whatsapp', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ wa_numero_dueno: num }),
      });
    } catch { /* red */ }
  }
  ...
  // Al abrir/recargar el panel: si vuelve de Mercado Pago (?recarga=...) abre
  // Recarga; si no, restaura la última sección donde estaba el usuario, para que
  // al dar "Refrescar app" o F5 se quede donde estaba y no vuelva al inicio.
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const params = new URLSearchParams(window.location.search);
    if (params.has('recarga')) { setActiveSection('recarga'); return; }
    // Enlace directo a una sección: /panel?seccion=plantillas (o ?tab=manual, etc.)
    const directa = (params.get('seccion') || params.get('tab') || '').trim();
    if (directa) { setActiveSection(directa as PanelSection); return; }
    try {
      const guardada = localStorage.getItem('panel_seccion');
      if (guardada) setActiveSection(guardada as PanelSection);
    } catch { /* sin localStorage: se queda en la sección por defecto */ }
  }, []);

  // Recuerda la sección actual para restaurarla en la próxima carga.
  useEffect(() => {
    try { localStorage.setItem('panel_seccion', activeSection); } catch { /* ignora */ }
  }, [activeSection]);
  ...
  const supabase = initBrowserSupabase(sbToken ?? null);
  ...
  return (
    ...
    <div data-medida={medida} className="panel-app flex h-full min-h-0 bg-[#FAF9F6] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        <PanelTabs activeSection={activeSection} onSectionChange={setActiveSection} />
        <div className="flex-1 min-h-0 flex overflow-hidden">
      {/* ── Chatbot: una sola bandeja (Funnel + WhatsApp), se filtra con las pestañas ── */}
      {(activeSection === 'chat' || activeSection === 'chat_ventas') && (
        <>
          <ConversationList
            ...
            duenoNumero={duenoNumero}
            onVincularDueno={guardarDueno}
          />
          <ChatArea
      ...
      {activeSection === 'dominio'       && <DominioPanel />}
      {activeSection === 'carritos'      && <CarritosAbandonados />}
      {activeSection === 'guias_effi'    && <GuiasEffiPanel />}
      ...
      {activeSection === 'integrar_ia'   && <IntegrarIaPanel />}
      {activeSection === 'entrenamiento' && <EntrenamientoPanel />}
      {activeSection === 'plantillas_conocimiento' && <PlantillasConocimientoPanel />}
      {activeSection === 'asistente_bot' && <AsistenteBotPanel />}
      {activeSection === 'entrenar_bot'  && <EntrenarBotPanel />}
      ...
      {activeSection === 'wa_config'     && <AjustesWaPanel />}
      {activeSection === 'recarga'       && <RecargaPanel />}
      {esSuperAdmin && activeSection === 'empresas' && <EmpresasPanel />}
      {esSuperAdmin && activeSection === 'quino_aprendizaje' && <AprendizajeQuinoPanel />}
      {activeSection === 'ajustes'       && <AjustesPanel />}
      {activeSection === 'manual'        && <ManualPanel />}
      {activeSection === 'plantillas_embudo' && <PlantillasEmbudoPanel />}
        </div>
      </div>

      {/* Quino: copiloto flotante en toda la app; sabe en qué sección estás parado */}
      <AsistenteConexion seccion={activeSection} />
    </div>
  );
}
