...
export async function reglasAprobadas(limite = 120, tenantId?: string | null): Promise<ReglaMemoria[]> {
  try {
 ...
 */
export async function bloqueDeMemoria(tenantId?: string | null): Promise<string> {
  const reglas = await reglasAprobadas(120, tenantId);
