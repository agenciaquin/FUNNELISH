 ...
 *  `destinos`: números a los que llega la ficha (el del dueño). Si no se pasa,
 *  se usan los de respaldo de la agencia. */
export async function mandarFichaVenta(supabase: any, pedido: any, tel10: string, from: string, destinos?: string[]) {
  if (!pedido) return;
  const receptores = (destinos && destinos.length) ? destinos : ADMINS_VENTAS;
  const registro =
  ...
  // Se envía a TODOS los destinos (el número del dueño, o respaldo agencia).
  for (const admin of receptores) {
    if (urls.length > 0) {
