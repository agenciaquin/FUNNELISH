 ...
 *
 * Se ejecuta por cron cada pocos minutos. Idempotente: al apagar limpia
 * `vendido_at`, así no se vuelve a procesar (y si el asesor reactiva el bot,
 * no se apaga otra vez).
 */
...
export async function GET(req: NextRequest) {
  ...
  // Ventas marcadas hace más de 30 min con el bot todavía encendido
  ...
  if (ids.length === 0) return NextResponse.json({ status: 'ok', apagados: 0 });
  ...
  return NextResponse.json({ status: 'ok', apagados: ids.length });
}
